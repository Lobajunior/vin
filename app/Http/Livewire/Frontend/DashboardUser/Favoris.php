...
class Favoris extends Component
{
    function deletedFavoris($id_produit)  {
        ...
        if($produit){
            $favoris = Fav::where('produit_id',$produit->id)->where('user_id',Auth::user()->id)->first();
                if(isset($favoris)){
                    $favoris->delete();
    ...
    {
        return view('livewire.frontend.dashboard-user.favoris',[
