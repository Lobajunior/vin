...
class LiveMoreAsk extends Component
{
    public $produitMoreAsk ;
    public $sum_rating_produit = array() ;

    function mount() : void {
       
        $this->produitMoreAsk = Produit::where('etat',1)->OrderBy('created_at','ASC')->where('is_djassa',0)->get();

        $table = [];

        //parcourt pour pouvoir utiliser l'id de chaque produit de la collection pour determiner ses notes 
        foreach($this->produitMoreAsk as $parameters){
    ...
    {
        return view('livewire.frontend.accueil.live-more-ask');
    }
