...
class LiveRecent extends Component
{
    ...
    public $produit_recent;
    public $sum_rating_produit = array() ;

    function mount() : void {
        
        $this->produit_recent = Produit::where('etat',1)->where('is_djassa',0)->OrderBy('created_at','DESC')->take(4)->get();

        $table = [];

        //parcourt pour pouvoir utiliser l'id de chaque produit de la collection pour determiner ses notes 
        foreach($this->produit_recent as $parameters){
    ...
    {
        return view('livewire.frontend.accueil.live-recent');
    }
}
